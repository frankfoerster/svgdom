...
describe('namespaces', () => {
  it('normalizes namespaces and validates qualified names', () => {
    ...
    assert(
      svgDocument.createElementNS(svg, 's:rect') instanceof SVGRectElement
    );
  ...
  it('parses fragment namespaces inherited from the host element', () => {
    ...
    assert.strictEqual(
      root.children[0].getAttributeNS('urn:q', 'value'),
      'yes'
    );
    ...
    assert.strictEqual(
      root.firstChild.firstChild.firstChild.namespaceURI,
      null
    );
  });
